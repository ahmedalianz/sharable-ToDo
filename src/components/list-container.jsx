...
export default function ListContainer({ list }) {
  ...
  const markListItem = async (listItem) => {
    try {
      ...
      if (listItem.marked) {
        await updateDoc(todoRef, { marked: false });
        const newList = listItems.filter((li) => li.id !== listItem.id);
        const index = listItems.indexOf(listItem);
        listItems[index].marked = false;
        setListItems([...newList, listItems[index]]);
        toast.success("List Item is un Marked");
      } else {
        await updateDoc(todoRef, { marked: true });
        const newList = listItems.filter((li) => li.id !== listItem.id);
        const index = listItems.indexOf(listItem);
        listItems[index].marked = true;
        setListItems([...newList, listItems[index]]);
  ...
  return isLoaded ? (
    <div className={`${list.selected}`}>
      <div className="card-body">
        ...
        <form
          className="d-flex justify-content-center align-items-center form-group mb-4"
          onSubmit={(e) => addListItem(e)}
        >
      ...
      <div
        className={classNames(
          "d-flex flex-column align-items-center p-3 bg-dark text-white",
          list ? "" : "d-none"
        )}
      >
        ...
        <Share listName={list.listName} userId={user.id} />
      </div>
    </div>
  ) : (
    <Spinner />
