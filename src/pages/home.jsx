  ...
  const addNewListTab = async (listName) => {
    try {
      if (listName !== "") {
        ...
        setAllLists([
          ...allLists,
          { id: list.id, listName, selected: "d-none" },
        ]);
        setNewListName("");
      ...
      try {
        ...
        querySnapshot.forEach((doc) => {
          if (doc.data().userId === user.id) {
            ...
            if (newAllLists.length > 0) {
              // to show only the first list at start up
        ...
        <div className="container py-5 h-100">
          <div className="row d-flex justify-content-center align-items-center h-100">
            <div className="col col-xl-10">
              <div className="card">
                <ul
                  className="nav nav-tabs mb-4 pb-2 position-relative"
                  id="ex1"
                  role="tablist"
                >
                  {allLists.map((list) => (
                  ...
                  <div className={`mx-2 ${newListTab}`}>
                    ...
                    />
                    <i
                      className="fas fa-check ms-2 cursor-pointer"
                      onClick={() => addNewListTab(newListName)}
                    ></i>
                  </div>
                  <div
                  ...
                  <i
                    ...
                    onClick={() => setNewListTab("d-flex align-items-center")}
                  ></i>
                </ul>
                {allLists.map((list) => (
